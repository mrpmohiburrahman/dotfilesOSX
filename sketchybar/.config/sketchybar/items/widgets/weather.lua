...
local weather_info = sbar.add("bracket", "widgets.weather.info", {weather_icon.name}, {
    background = {
        color = colors.bg2,
...
local function fetch_weather_data(callback)
    local command = "~/dotfilesOSX/sketchybar/sketchybar-scripts/akt_weather.sh"
...
local function toggle_details()
    ...
    if should_draw then
        ...
        sbar.exec("~/dotfilesOSX/sketchybar/sketchybar-scripts/akt_weather.sh", function(result)
            ...
            temperature:set({
                label = popup_temperature .. " °C"
            })
            condition:set({
                label = popup_condition
            })
            windspeed:set({
                label = popup_wind_speed .. " km/h"
            })
            cloudcover:set({
                label = popup_cloud_cover .. " %"
            })
            visibility:set({
                label = string.format("%.1f km", visibility_km)
            })
        end)
    else
        hide_details()
