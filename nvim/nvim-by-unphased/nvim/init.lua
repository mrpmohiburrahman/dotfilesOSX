...
function enhanced_dot(count)
    ...
    local c = count
    local searchInfo = vim.fn.searchcount({
        maxcount = 9999
    })
...
vim.api.nvim_set_keymap('n', '<M-.>', ":lua enhanced_dot(vim.v.count1)<CR>", {
    noremap = true
})
vim.api.nvim_set_keymap('n', '<M->>', ":lua enhanced_dot('')<CR>", {
    noremap = true
})
...
local function filter_to_real_wins(window_list)
    ...
    for _, win in ipairs(window_list) do
        ...
        if vim.api.nvim_win_get_config(win).focusable and buftype ~= "nofile" and buftype ~= "nowrite" and buftype ~=
            "help" and buftype ~= "quickfix" then
...
_G.CycleWindowsOrBuffers = function(forward)
    ...
    elseif not forward and curwin == wins_in_curtab[1] then
        ...
        log("CycleWindowsOrBuffers in the last case (multiple tabs, not at end), cycling window " ..
                (forward and "forward" or "backward"))
        vim.cmd("wincmd " .. (forward and "w" or "W"))
    end
end
...
local function tmux_window(dir)
    ...
    if target_win ~= vim.fn.winnr() then -- has somewhere to go
        ...
        vim.system({'tmux', 'select-pane', '-' .. string.gsub(dir, '[hjkl]', {
            h = 'L',
            j = 'D',
            k = 'U',
            l = 'R'
...
vim.keymap.set({"n", "v"}, "<C-h>", function()
    ...
    noremap = true,
    desc = "Move to window on left, overflow to tmux"
})
vim.keymap.set({"n", "v"}, "<C-j>", function()
    tmux_window('j')
end, {
    noremap = true,
    desc = "Move to window below, overflow to tmux"
})
vim.keymap.set({"n", "v"}, "<C-k>", function()
    tmux_window('k')
end, {
    noremap = true,
    desc = "Move to window above, overflow to tmux"
})
vim.keymap.set({"n", "v"}, "<C-l>", function()
    tmux_window('l')
end, {
    noremap = true,
    desc = "Move to window on right, overflow to tmux"
})
...
vim.api.nvim_set_keymap('n', 'I', ':lua smart_insert_start_of_line()<CR>', {
    noremap = true,
    silent = true
})
...
safeRequire("gitsigns").setup({
    on_attach = function(bufnr)
        ...
        map('n', '<leader>d', function()
            ...
            expr = true,
            desc = "Toggle Show Deleted (GitSigns)"
        })
    end,
    diff_opts = {
        internal = true
...
vim.keymap.set("n", "<leader><CR>", telescope_builtin.grep_string, {
...
vim.keymap.set("n", "<M-P>", function()
    telescope_builtin.command_history({
        filter_fn = function(cmd)
            local patterns = {"^w[aq;]?$", "^qa?$", "^e$", "^mes$", "^h ", "^map ?", "^%d+$"}
...
safeRequire("nvim-autopairs").setup({
    map_cr = false
})
...
vim.keymap.set("n", "<leader>y", safeRequire("osc52").copy_operator, {
    expr = true,
    desc = "Copy to host term system clipboard via OSC 52"
})
vim.keymap.set("n", "<leader>yy", "<leader>y_", {
    remap = true,
    desc = "Copy entire line to host term system clipboard via OSC 52"
})
vim.keymap.set("x", "<leader>y", safeRequire("osc52").copy_visual, {
    desc = "Copy visual selection to host term system clipboard via OSC 52"
})
vim.keymap.set("n", "<leader>Y", "ggVG<leader>y", {
    remap = true,
    desc = "Copy entire buffer to host clipboard via OSC 52"
})
    ...
    handlers = {
        ...
        pylint = function(source_name, methods)
            ...
            null_ls.register(null_ls.builtins.diagnostics.pylint.with({
                env = function(params)
                    local computed_pythonpath = vim.fn.system(
...
null_ls.setup({
    ...
    sources = {null_ls.builtins.code_actions.gitsigns, null_ls.builtins.code_actions.ts_node_action}
})
...
local lsp_attach = function(x, bufnr)
    ...
    vim.keymap.set('n', '<leader>ca', vim.lsp.with(vim.lsp.buf.code_action, {
        border = nil
    }), ext(bufopts, "desc", "Code Action Menu"))
    ...
    end, ext(bufopts, "desc", "Format Buffer"))
end
...
safeRequire("mason-lspconfig").setup_handlers {
    ...
    ["tsserver"] = function()
        log "exception applied for mason lspconfig setup for tsserver as we want to use typescript-tools instead."
    end,
...
vim.keymap.set('n', "<leader>T", ":OnlineThesaurusCurrentWord<CR>", {
...
vim.keymap.set({"n", "o", "x"}, "w", "<cmd>lua require('spider').motion('w')<CR>", {
    desc = "Spider-w (hit sub words, skip to word chars)"
})
vim.keymap.set({"n", "o", "x"}, "e", "<cmd>lua require('spider').motion('e')<CR>", {
    desc = "Spider-e (hit sub words, skip to word chars)"
})
vim.keymap.set({"n", "o", "x"}, "<BS>", "<cmd>lua require('spider').motion('b')<CR>", {
    desc = "Spider-b (hit sub words, skip to word chars)"
})
...
vim.api.nvim_set_keymap("n", "<leader>R", "<cmd>lua require('telescope').extensions.refactoring.refactors()<CR>", {
    noremap = true,
    desc = "refactors ()"
})
vim.api.nvim_set_keymap("v", "<leader>r", "<Esc><cmd>lua require('telescope').extensions.refactoring.refactors()<CR>", {
    noremap = true,
    desc = "Open refactoring menu in visual mode"
})
...
require('lspconfig').emmet_language_server.setup({
    filetypes = {"css", "eruby", "html", "markdown", "javascript", "javascriptreact", "less", "sass", "scss", "svelte",
                 "pug", "typescript", "typescriptreact", "vue"},
    init_options = {
...
vim.keymap.set('n', 'X', ':BD<CR>', {
    desc = "Delete current buffer with vim-bufkill"
})
...
vim.keymap.set('n', '<leader>cb', ':lua CloseAllBufsNotOpenInTabs()<CR>', {
    desc = "Close all buffers not already open in tabs"
})
vim.keymap.set('n', '<leader>cc', ':lua FlattenAllWindows()<CR>', {
    desc = "close all other buffers than windows open in tabs and go to buffer workflow"
})

vim.keymap.set('i', '<M-NL>', '<CR>', {
    noremap = true,
    desc = "also newline"
})
...
local function nvim_state_update(ev)
    vim.schedule(function()
        local cmd = {vim.env.HOME .. "/util/nvim-update-win.sh", tostring(vim.fn.getpid()), vim.v.servername,
                     vim.fn.getcwd(), ev.file, ev.event}
...
vim.keymap.set('n', '<leader>z', ':lua toggle_scrolloff()<CR>', {
...
vim.keymap.set('x', 'gci', ':normal gcc<CR>', {
