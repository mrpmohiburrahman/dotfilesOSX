-- .config/sketchybar/items/widgets/ssd.lua
...
local ssd_bracket = sbar.add("bracket", "widgets.ssd.bracket", {ssd.name, ssd_volume.name}, {
    background = {
        color = colors.bg2,
...
system_info_swap:subscribe("swap_system_info", function(env)
    ...
    if drawing then
        sbar.set("widgets.wifi1", {
            drawing = false
        })
        sbar.set("widgets.wifi2", {
            drawing = false
        })
        sbar.set("/ram\\.*/", {
            drawing = false
